...
module.exports = function define(schema) {
  var keys = Object.keys(schema)
    ...
    , mapsByAttr = flatPick(schema, 'map')
    , validatorsByAttr = flatPick(schema, 'validators')
    , Model;

  // ensure maps are callbackified
  mapsByAttr = _(mapsByAttr).reduce(function(memo, val, key) {
    memo[key] = cbify(val);
    return memo;
  }, {});

  // ensure validators are callbackified and contained in an array
  validatorsByAttr = _(validatorsByAttr).reduce(function(memo, val, key) {
    if (_.isArray(val)) {
